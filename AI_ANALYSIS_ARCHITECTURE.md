...
Synthetic unit coverage covers exact wealth differences, bank-balance mismatches, multi-year funds tracing, local transaction parsing, capital-gains worksheet arithmetic, asset and liability continuity, strict extraction and review schemas, prompt-injection boundaries, and the server-owned chatbot prompt. The current full project suite passed **29 test files and 138 tests**. TypeScript checking passed. The production Vite and server build passed. A live preview probe using only synthetic figures returned HTTP 200 with both a review object and a deterministic calculation pack; the synthetic response content was not retained.
...
This is an evolving architecture, not a complete tax-law knowledge platform. The project now includes a dated official-source catalogue, local Excel/CSV-style transaction parsing, high-level authenticated draft support, and a preparation-oriented capital-gains/property worksheet. It does not yet provide live FBR retrieval, a fully versioned source database, OCR-specific preprocessing, full transaction de-duplication, property notification search, or statutory capital-gain tax calculators. Those features should be added separately with dated official sources, explicit tax-year applicability, deterministic formulas, migration tests, and a reviewed privacy design.

The next safe expansion is to formalize the existing dated source catalogue into a versioned, source-linked knowledge record for a small set of official FBR destinations, followed by carefully scoped OCR preprocessing or stronger transaction de-duplication. Any additional document types or statutory calculators should be introduced only after synthetic validation, tax-year review, and an explicit privacy design.