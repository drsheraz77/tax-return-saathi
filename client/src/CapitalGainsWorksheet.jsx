...
import { TAX_YEAR_CAPITAL_GAINS, TAX_YEAR_CAPITAL_GAINS_BOUNDARY, TAX_YEAR_CAPITAL_GAINS_REVIEWED_ON, TAX_YEAR_CAPITAL_GAINS_SOURCES } from "./capitalGainsSources.js";
...
export default function CapitalGainsWorksheet() {
  ...
  return <div>
    ...
    <details className="reconciliation__summary reconciliation__sources"><summary><strong>Official {TAX_YEAR_CAPITAL_GAINS} source notes / سرکاری سورس نوٹس</strong></summary><p><strong>Reviewed:</strong> {TAX_YEAR_CAPITAL_GAINS_REVIEWED_ON}. These references explain the filing structure and legal starting points behind the worksheet; they do not turn the worksheet into a tax calculator or live legal database.</p><p lang="ur" dir="rtl"><strong>جائزہ:</strong> {TAX_YEAR_CAPITAL_GAINS_REVIEWED_ON}۔ یہ حوالے فائلنگ ڈھانچے اور قانونی ابتدائی ماخذ کی وضاحت کرتے ہیں؛ یہ ورک شیٹ کو ٹیکس کیلکولیٹر یا لائیو قانونی ڈیٹا بیس نہیں بناتے۔</p><ol>{TAX_YEAR_CAPITAL_GAINS_SOURCES.map((source) => <li key={source.id}><a href={source.url} target="_blank" rel="noreferrer">{source.sourceLabel} ↗</a><br /><strong>{source.title}</strong><p>{source.note}</p><p lang="ur" dir="rtl">{source.noteUrdu}</p><p><em>Worksheet use:</em> {source.worksheetUse}</p><p lang="ur" dir="rtl"><em>ورک شیٹ میں استعمال:</em> {source.worksheetUseUrdu}</p></li>)}</ol><p><strong>Boundary:</strong> {TAX_YEAR_CAPITAL_GAINS_BOUNDARY.english}</p><p lang="ur" dir="rtl"><strong>حد:</strong> {TAX_YEAR_CAPITAL_GAINS_BOUNDARY.urdu}</p></details>
